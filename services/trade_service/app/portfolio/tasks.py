...
from services.trade_service.app.portfolio.service import get_performance, get_portfolio_snapshot

logger = get_logger("portfolio_tasks")


@celery_app.task(name="trade_service.tasks.generate_daily_report", bind=True, max_retries=2)
def generate_daily_report(self, trading_date: str | None = None) -> dict:
    return run_async(_generate_daily_report_async(trading_date))
