...
async def trigger_manual_pipeline(req: ManualPipelineRequest):
    ...
    if td == today and is_market_open():
        settings = get_settings()
        mkt_end = settings.common.market_hours.end
        raise HTTPException(
            status_code=422,
            detail={
                "error": (
                    f"Market is currently open ({now_market().strftime('%H:%M')}, closes {mkt_end}). "
                    f"Pipeline requires market close. Run after {mkt_end}, or use a past trading_date."
                ),
            },
        )

    task = celery_app.send_task(
        "data_service.tasks.run_manual_pipeline",
        args=[parsed, td_str, req.stop_after],
        queue="data",
    )
    ...
    return CollectResponse(
        ...
        status="queued",
        message=(
            f"Manual pipeline queued for {len(parsed)} symbols ({', '.join(parsed)}), "
            f"trading_date={td_str}"
        ),
    )
