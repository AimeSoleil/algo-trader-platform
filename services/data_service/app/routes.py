...
# ── Post-market data collect trigger ───────────────────────


class PostMarketCollectRequest(BaseModel):
    """Trigger post-market data collection (1m bars + daily bars + aggregate option daily)."""
    trading_date: date | None = None


class PostMarketCollectResponse(BaseModel):
    task_id: str
    status: str = "queued"
    trading_date: str
    message: str = ""
...
async def trigger_post_market_collect(req: PostMarketCollectRequest | None = None):
    """手动触发盘后数据采集：1m bars + daily bars + aggregate option daily。

    不会触发 backfill / signals / blueprint，仅执行数据采集部分。
    ...
    # Only validate market hours for today's date
    if td == today:
        if before_market_open():
            raise HTTPException(
                status_code=422,
                detail={
                    "error": (
                        f"Market has not opened yet ({now_market().strftime('%H:%M')}). "
                        f"Post-market collection requires market close."
                    ),
                    "suggestion": f"Use trading_date={previous_trading_day(today).isoformat()} for yesterday's data.",
                },
            )
        if is_market_open():
            raise HTTPException(
                ...
                detail={
                    "error": (
                        f"Market is currently open ({now_market().strftime('%H:%M')}, closes {mkt_end}). "
                        f"Post-market data is only complete after market close."
                    ),
                    "suggestion": f"Run after {mkt_end}, or use a past trading_date.",
                },
            )
    ...
    task = celery_app.send_task(
        "data_service.tasks.collect_post_market_data",
        args=[td_str],
        queue="data",
    )

    return PostMarketCollectResponse(
        task_id=task.id,
        status="queued",
        trading_date=td_str,
        message=(
            f"Post-market data collection queued for {td_str}: "
            f"1m bars → daily bars → aggregate option daily"
        ),
    )
