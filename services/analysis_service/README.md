...
符号语义：Analysis Service 的自动 daily 流程会直接分析 `common.watchlist.for_data_signal`。其中 `for_trade_benchmark` 作为 benchmark context 注入每个 chunk，但因为它必须属于 `for_data_signal`，所以这些 benchmark symbol 仍然可以生成 `symbol_plan`。
...
Automatic daily pipeline 的 analysis stage 现在分两层：

- 外层 Celery fan-out：`for_data_signal` 按 `analysis_service.daily_task_chunk_size` 分块，默认每 20 个 symbol 一个 analysis task
- 内层 orchestrator：单个 chunk 内仍可按 `analysis_service.llm.orchestrator_chunk_size` 做上下文保护型分块
- `for_trade_benchmark` 会作为 shared market snapshot 注入每个 chunk
- 全部 chunk 完成后，结果会统一 merge、去重，并输出一个最终 blueprint
- orchestrator 内部并发仍受 `analysis_service.llm.orchestrator_max_parallel` 限制
