...
health_watchdog() {
  ...
  local worker_names=("data" "backfill" "signal" "analysis")
...
run_with_restart data \
  $CELERY_CMD worker -Q data -n data@%h \
  --loglevel=INFO --logfile="$LOG_DIR/celery-data.log" &
WRAPPER_PIDS[data]=$!

run_with_restart backfill \
  $CELERY_CMD worker -Q backfill -n backfill@%h \
  --loglevel=INFO --logfile="$LOG_DIR/celery-backfill.log" &
WRAPPER_PIDS[backfill]=$!

run_with_restart signal \
  $CELERY_CMD worker -Q signal -n signal@%h \
  --loglevel=INFO --logfile="$LOG_DIR/celery-signal.log" &
WRAPPER_PIDS[signal]=$!

run_with_restart analysis \
  $CELERY_CMD worker -Q analysis -n analysis@%h \
  --loglevel=INFO --logfile="$LOG_DIR/celery-analysis.log" &
WRAPPER_PIDS[analysis]=$!
